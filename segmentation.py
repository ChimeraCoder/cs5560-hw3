...
"""Return the costs for each pixel, which are 1 if the pixel brightness is above the prc^th percentile, and 0 otherwise"""
def costForImage(image, percentile):
    prc = np.percentile(image, percentile)
    return (image > prc).astype(int)
...
# Set up a particularly dumb unary cost (is 1 if first image is nonzero)
    ...
    #unary0 = (images[0] > prc).astype(int)
    unary0 = sum([costForImage(image, 93) for image in images[:6]])
    print(unary0)
